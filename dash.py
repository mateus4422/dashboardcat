...
if "Prioridade" in df.columns:
    ...
    if "Selecionar todos" not in lojas_selecionadas:
        ...
        dados_lojas_selecionados = dados_status_selecionado if menu_selecionado == "Prioridade" else df

    # Organizar os blocos de total em uma grade
    total_container = st.container()
    total_block = total_container.columns(5)
    ...
    with total_block[0]:
        ...
        total_faturamento_st = dados_lojas_selecionados["Faturamento ST"].sum()
    ...
    with total_block[1]:
        ...
        total_ressarcimento = dados_lojas_selecionados["Ressarcimento"].sum()
    ...
    with total_block[2]:
        ...
        total_complemento = dados_lojas_selecionados["Complemento"].sum()
    ...
    if status_selecionado == "Em Desenvolvimento":
        with total_block[4]:
            ...
            if not dados_lojas_selecionados.empty:
                media_percentual_ressarcimento = dados_lojas_selecionados["% Ressarcimento"].mean() * 100
    ...
    st.bar_chart(dados_lojas_selecionados.set_index("Loja")["Faturamento ST"], use_container_width=True)

    # Gráfico de Barras (Ressarcimento)
    st.subheader("Gráfico de Barras (Ressarcimento)")
    st.bar_chart(dados_lojas_selecionados.set_index("Loja")["Ressarcimento"], use_container_width=True)

    # Gráfico de Barras (Complemento)
    st.subheader("Gráfico de Barras (Complemento)")
    st.bar_chart(dados_lojas_selecionados.set_index("Loja")["Complemento"], use_container_width=True)
