...
with total_block1:
    ...
    st.markdown(f'<div style="{value_style}">R$ {total_faturamento_st:,.2f}</div>', unsafe_allow_html=True)

# Bloco de Ressarcimento
with total_block2:
    st.subheader("Total Ressarcimento")
    total_ressarcimento = dados_lojas_selecionadas["Ressarcimento"].sum()
    st.markdown(f'<div style="{value_style}">R$ {total_ressarcimento:,.2f}</div>', unsafe_allow_html=True)

# Bloco de Complemento
with total_block3:
    st.subheader("Total Complemento")
    total_complemento = dados_lojas_selecionadas["Complemento"].sum()
    st.markdown(f'<div style="{value_style}">R$ {total_complemento:,.2f}</div>', unsafe_allow_html=True)
...
with total_block4:
    ...
    st.markdown(f'<div style="{value_style}">R$ {diferenca_ressarcimento_complemento:,.2f}</div>', unsafe_allow_html=True)

# Mostrar a média de ressarcimento
media_percentual_ressarcimento = ((dados_lojas_selecionadas["Ressarcimento"] - 100) / dados_lojas_selecionadas["Faturamento ST"]).mean()
