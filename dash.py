...
# Criar um menu suspenso para selecionar entre "Prioridade de Lojas" e "Geral"
menu_selecionado = st.selectbox("Selecione o Menu:", ["Prioridade de Lojas", "Geral"])

# Filtrar dados com base na seleção do menu
if menu_selecionado == "Prioridade de Lojas":
    dados_menu = df[df["Prioridade"] == "Sim"]
else:
    dados_menu = df[df["Prioridade"] != "Sim"]

# Resto do código permanece o mesmo, usando os dados filtrados em "dados_menu"
...
with total_block[0]:
    ...
    total_faturamento_st = dados_menu["Faturamento ST"].sum()
...
with total_block[1]:
    ...
    total_ressarcimento = dados_menu["Ressarcimento"].sum()
...
with total_block[2]:
    ...
    total_complemento = dados_menu["Complemento"].sum()
...
with total_block[4]:
    ...
    if menu_selecionado == "Prioridade de Lojas":
        ...
        # Certifique-se de que a variável dados_menu foi definida
        if "dados_menu" in locals():
            media_percentual_ressarcimento = dados_menu["% Ressarcimento"].mean() * 100
        else:
...
st.bar_chart(dados_menu.set_index("Loja")["Faturamento ST"], use_container_width=True)

# Gráfico de Barras (Ressarcimento)
st.subheader("Gráfico de Barras (Ressarcimento)")
st.bar_chart(dados_menu.set_index("Loja")["Ressarcimento"], use_container_width=True)

# Gráfico de Barras (Complemento)
st.subheader("Gráfico de Barras (Complemento)")
st.bar_chart(dados_menu.set_index("Loja")["Complemento"], use_container_width=True)
